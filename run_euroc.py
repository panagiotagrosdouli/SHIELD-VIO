...
from shield_vio.slam.sparse_map import SparseLandmarkMap
...
def build_stereo_pnp_provider(
    sequence_root: Path, *, use_persistent_map: bool = False
) -> StereoPnPVisualProvider:
    ...
    landmark_map = SparseLandmarkMap() if use_persistent_map else None
    return StereoPnPVisualProvider(
        frontend, left.transform_body_sensor, landmark_map=landmark_map
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--camera",
        default="cam0",
        help="EuRoC camera stream used for frame timestamps",
    )
    ...
    visual_group.add_argument(
        "--stereo-map",
        action="store_true",
        help="Build a persistent stereo map and fuse global map-PnP pose updates",
    )
    parser.add_argument(
        "--skip-evaluation",
        action="store_true",
        help="Do not compare against EuRoC ground truth",
    )
    parser.add_argument(
        "--max-gap",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="RPE displacement interval in associated samples",
    )
    parser.add_argument(
        "--with-scale",
        action="store_true",
        help="Use Sim(3) instead of SE(3) alignment",
    )
    ...
    elif args.stereo_pnp or args.stereo_map:
        if args.camera != "cam0":
            parser.error(
                "--stereo-pnp/--stereo-map require --camera cam0 because cam1 is the paired right stream"
            )
        visual_provider = build_stereo_pnp_provider(
            args.sequence_root, use_persistent_map=args.stereo_map
        )
