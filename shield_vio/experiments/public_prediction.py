...
def _ground_truth_positions(path: Path) -> tuple[np.ndarray, np.ndarray]:
    timestamps: list[int] = []; positions: list[list[float]] = []
    with path.open("r", encoding="utf-8", newline="") as stream:
        reader = csv.reader(line for line in stream if not line.lstrip().startswith("#"))
        for row in reader:
            if not row: continue
            try:
                timestamps.append(int(row[0].strip())); positions.append([float(row[1]), float(row[2]), float(row[3])])
            except (ValueError, IndexError) as exc: raise ValueError(f"invalid ground-truth row in {path}: {row}") from exc
    ts = np.asarray(timestamps, dtype=np.int64); xyz = np.asarray(positions, dtype=float)
    if len(ts) < 2 or np.any(np.diff(ts) <= 0): raise ValueError("ground-truth timestamps must be strictly increasing")
    return ts, xyz
...
def _nearest_position_error(timestamps: np.ndarray, estimate: np.ndarray, gt_ts: np.ndarray, gt_xyz: np.ndarray, max_gap_ns: int) -> np.ndarray:
    ...
    for index, timestamp in enumerate(timestamps):
        right = int(np.searchsorted(gt_ts, timestamp)); candidates = [c for c in (right - 1, right) if 0 <= c < len(gt_ts)]
        if not candidates: continue
        best = min(candidates, key=lambda c: abs(int(gt_ts[c]) - int(timestamp)))
        if abs(int(gt_ts[best]) - int(timestamp)) <= max_gap_ns: result[index] = float(np.linalg.norm(estimate[index] - gt_xyz[best]))
    return result
...
def build_public_prediction_artifacts(run_dir: str | Path, ground_truth_csv: str | Path, output_dir: str | Path, *, dataset: str, sequence: str, estimator: str, failure_config: str | Path, seed: int = 0, max_ground_truth_gap_seconds: float = 0.02, evidence_level: str = "FIXTURE_INTEGRATION_TEST") -> dict[str, object]:
    if evidence_level not in ALLOWED_EVIDENCE_LEVELS: raise ValueError(f"invalid evidence level: {evidence_level}")
    source = Path(run_dir); destination = Path(output_dir); destination.mkdir(parents=True, exist_ok=True)
    health_rows = _dict_rows(source / "health.csv"); trajectory_rows = _dict_rows(source / "trajectory.csv")
    if len(health_rows) != len(trajectory_rows) or len(health_rows) < 2: raise ValueError("health and trajectory artifacts must align and contain at least two rows")
    timestamps = np.asarray([int(row["frame_timestamp_ns"]) for row in health_rows], dtype=np.int64)
    if np.any(np.diff(timestamps) <= 0): raise ValueError("public run timestamps must be strictly increasing")
    ...
    if tuple(c.name for c in definition.criteria) != ("position_error_m",): raise ValueError("public smoke builder only accepts the declared position-error sensitivity definition")
    ...
    dataset_obj = build_prediction_dataset(samples, horizon_targets={h: t.labels for h, t in targets.targets.items()}, eligible_masks={h: t.eligible_mask for h, t in targets.targets.items()}, groups=PredictionGroups(dataset, sequence, estimator, "clean", seed), metadata={"evidence_level": evidence_level, "confirmatory": False})
    values, names = dataset_obj.features()
    with (destination / "prediction_dataset.csv").open("w", encoding="utf-8", newline="") as stream:
        writer = csv.writer(stream); writer.writerow(["timestamp_ns", *names]); writer.writerows([[int(t), *row.tolist()] for t, row in zip(timestamps, values)])
    with (destination / "failure_events.csv").open("w", encoding="utf-8", newline="") as stream:
        writer = csv.writer(stream); writer.writerow(["event_id", "onset_ns", "offset_ns"]); writer.writerows(zip(events.event_ids, events.onsets_ns.tolist(), events.offsets_ns.tolist()))
