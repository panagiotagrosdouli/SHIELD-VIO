"""Map real public estimator artifacts into the Phase B canonical prediction pipeline."""
...
def build_public_prediction_artifacts(
    run_dir: str | Path,
    ...
    *,
    ...
    for timestamp, health, error in zip(timestamps, health_rows, position_error):
        ...
        samples.append(HealthSample(
            timestamp_ns=int(timestamp), sequence_id=sequence, estimator_id=estimator,
            estimator=EstimatorHealth(
                nis=HealthValue.missing() if nis_text == "" else HealthValue(float(nis_text), True, int(timestamp)),
                covariance_trace=HealthValue(float(health["covariance_trace"]), True, int(timestamp)),
                covariance_condition_number=HealthValue(float(health["covariance_condition_number"]), True, int(timestamp)),
            ),
            evaluation=EvaluationDiagnostics(
                ground_truth_position_error_m=HealthValue.missing() if not np.isfinite(error) else HealthValue(float(error), True, int(timestamp))
            ),
        ))
    ...
    dataset_obj = build_prediction_dataset(
        samples,
        horizon_targets={h: target.labels for h, target in targets.targets.items()},
        eligible_masks={h: target.eligible_mask for h, target in targets.targets.items()},
        groups=PredictionGroups(dataset, sequence, estimator, "clean", seed),
        metadata={"evidence_level": "PUBLIC_DATASET_SMOKE", "confirmatory": False},
    )
    values, names = dataset_obj.features()
    with (destination / "prediction_dataset.csv").open("w", encoding="utf-8", newline="") as stream:
        writer = csv.writer(stream); writer.writerow(["timestamp_ns", *names])
        for timestamp, row in zip(timestamps, values): writer.writerow([int(timestamp), *row.tolist()])
    with (destination / "failure_events.csv").open("w", encoding="utf-8", newline="") as stream:
        writer = csv.writer(stream); writer.writerow(["event_id", "onset_ns", "offset_ns"])
        writer.writerows(zip(events.event_ids, events.onsets_ns.tolist(), events.offsets_ns.tolist()))
    manifest = {
        "evidence_level": "PUBLIC_DATASET_SMOKE", "confirmatory": False,
        "dataset": dataset, "sequence": sequence, "estimator": estimator,
        "health_schema_version": samples[0].schema_version,
        "failure_definition_id": definition.schema_version,
        "prediction_horizons_seconds": list(definition.horizons_seconds),
        "samples": len(samples), "failure_events": len(events.event_ids),
        "ground_truth_associated_samples": int(np.sum(np.isfinite(position_error))),
