...
def test_accepts_verified_geometry_and_builds_metric_loop_edge(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_rejects_degenerate_geometry(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        module, "estimate_relative_pose", lambda *args, **kwargs: _measurement(degenerate=True)
    )
    result = module.verify_loop_candidate(
        100, 5, np.zeros((30, 2)), np.zeros((30, 2)), np.eye(3), translation_scale_m=1.0
    )
...
def test_rejects_excessive_epipolar_error(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        module, "estimate_relative_pose", lambda *args, **kwargs: _measurement(epipolar=2.0)
    )
    result = module.verify_loop_candidate(
        100,
        5,
...
def test_rejects_invalid_scale_and_self_loop() -> None:
    with pytest.raises(ValueError, match="different frames"):
        module.verify_loop_candidate(
            5, 5, np.zeros((8, 2)), np.zeros((8, 2)), np.eye(3), translation_scale_m=1.0
        )
    with pytest.raises(ValueError, match="translation_scale"):
        module.verify_loop_candidate(
            6, 5, np.zeros((8, 2)), np.zeros((8, 2)), np.eye(3), translation_scale_m=0.0
        )
