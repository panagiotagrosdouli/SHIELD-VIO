...
def test_public_run_maps_to_phase_b_canonical_features_without_ground_truth_leakage(tmp_path: Path) -> None:
    ...
    _write(run / "health.csv", ["frame_timestamp_ns", "state_timestamp_ns", "initialized", "tracking_status", "propagated_imu_samples", "covariance_trace", "covariance_condition_number", "innovation_nis"],
           [[t, t, True, "tracking", i, 1.0 + i, 2.0, 3.0] for i, t in enumerate(timestamps)])
    _write(run / "trajectory.csv", ["frame_timestamp_ns", "state_timestamp_ns", "px", "py", "pz"],
           [[t, t, 0.0 if i < 2 else 2.0, 0.0, 0.0] for i, t in enumerate(timestamps)])
    gt = tmp_path / "gt.csv"
